sap.ui.define([
    ...
    return Controller.extend("project.controller.account", {
      ...
      onInit: function () {
        ...
        console.log("Loading customer with id:", userId);
  
        var oFilter = new Filter("id", FilterOperator.EQ, userId);
  
        oModel.read("/Customer", {
          filters: [oFilter],
          success: function (oData) {
            if (oData.results && oData.results.length > 0) {
              var oCustomerModel = new sap.ui.model.json.JSONModel(oData.results[0]);
              oView.setModel(oCustomerModel, "customer");

            } else {
              MessageToast.show("No customer data found for user.");
            }
          },
          error: function (oError) {
            MessageToast.show("Failed to load customer data.");
            console.error("Customer data load error:", oError);
          }
        });
      },
        ...
        var oDialog = new sap.m.Dialog({
          ...
          beginButton: new sap.m.Button({
            ...
            press: function () {
              ...
              // Send update request
              oModel.update("/CustomerSet('" + userId + "')", {address : newAddress}, {
                success: function () {
